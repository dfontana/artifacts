...
| --- | --- | --- | --- |
| 1 | `Intent` enum | `core/src/step.rs:35-42` | Generated by `define_intents!` in new `core/src/wire.rs`; re-exported from `step.rs` so external paths don't change. |
...
| 7 | `register_run_host_fns` `&str` match + plan-context stub loop | `src/lua.rs:433-483` and `src/lua.rs:395-403` | Replaced by one `host_fn!` row list that serves **both** the live and plan-context registrations (§5). |
...
| # | Decision | Choice | Why |
| --- | --- | --- | --- |
| 1 | Enum variant shape | **Tuple variants wrapping per-intent wire structs**: `Intent::Move(wire::Move)`, not named fields on the enum | Makes the dispatch macro trivial (`Self::$v(w) => w.request()`), keeps `Intent` small/`Clone`/exhaustively matched, and puts the payload type and its behavior on one struct. Construction churn is 11 sites, all listed in §2. |
| 2 | Macro vs. hand-written dispatch | **`macro_rules!` table (`define_intents!`) generating the enum + two delegating matches** | One row per intent; a row whose wire struct lacks an `IntentWire` impl fails to compile. No proc-macro crate (not earned at ~6 intents; every expansion is `cargo expand`-readable). |
| 3 | Where per-intent code lives | **New module `core/src/wire.rs`**: trait, payload structs, per-intent structs + impls, macro, invocation. `step.rs` re-exports `Intent`. | One file holds everything per-intent on the `core` side. Re-export keeps `artifacts_core::step::Intent` working so `character.rs`/`scheduler.rs` imports don't change. |
| 4 | `OutcomeKind` generation | **Stays a hand-written enum in `step.rs`** | It is not 1:1 with intents: `NoOp` is constructed by the scheduler with no intent, and `Deposit` serves both `deposit_item` and the `deposit_all` loop. A wire impl returning a nonexistent variant is a compile error, so hand-maintenance is already safe; macro-generating it would double macro complexity for zero safety gain. |
...
| 6 | Host-fn side | **One `host_fn!` row list inside `register_run_host_fns(lua, host, Option<Character>)`, used by both live and plan-context paths** | Host-fn binding is inherently runtime (closures per `Lua` state), so there is no compile-time exhaustiveness to lose — the win is one list instead of two. Taking `Option<Character>` makes the plan-context stubs *the same rows* by construction: a row can never be live-only or stub-only. |
...
**Revision message:** `refactor(core): one-place intent definitions (wire.rs + define_intents!)`
...
//! together on one struct, and `define_intents!` generates the `Intent` enum
//! plus both dispatch methods from one row list. Adding an intent on the
//! `core` side is: one struct + `IntentWire` impl below, one row in the
//! `define_intents!` invocation, and (if it produces a new outcome shape) one
//! `OutcomeKind` variant in `step.rs` — all compile-checked: a row without an
//! impl, or an impl naming a missing variant, fails to build, never to run.
//!
...
/// classification) stays in `machine.rs`.
...
/// Generate the `Intent` enum and its two dispatch methods from one row list.
/// Each variant wraps its wire struct; the matches delegate to `IntentWire`,
/// so a row without a corresponding impl fails to compile, never to run.
    ...
    ($(#[$meta:meta])* $vis:vis enum $name:ident { $($variant:ident),* $(,)? }) => {
        ...
        impl $name {
            ...
            pub(crate) fn outcome(&self, payload: ActionPayload) -> OutcomeKind {
                match self {
                    $( Self::$variant(w) => w.outcome(payload), )*
                }
            }
        }
    };
...
define_intents! {
    ...
    /// (craft, equip, use, recycle, …) get a row when their host fn lands,
    /// not before — untested wire-format code only rots.
    pub enum Intent { Move, Gather, Fight, Rest, DepositItem }
}
...
**Revision message:** `refactor: fold RUN_HOST_FNS + its match into one host_fn! row list`
...
```rust
    ...
    // plan context (no character) the SAME rows register as loud stubs — one
    // list by construction, so the two paths can't drift.
...
```rust
/// Register every run-pass host fn. The `host_fn!` rows below are the single
/// list of run fns: with a live `Character` each row binds its real closure;
/// with `None` (plan context) the same row registers a closure that errors
/// loudly when called. Adding an intent's binding is one row here — there is
/// no separate name list to keep in sync.
fn register_run_host_fns(
    ...
    // host_fn!("name", |c, lua_binding, arg: Ty, ...| body): one
    // `lua.create_function` + `host.set` pair. `c` is the live `&Character`;
    // in plan context the closure raises before `body` runs. Name the second
    // binding `_lua` when the body doesn't need the Lua handle.
    macro_rules! host_fn {
        ($name:literal, |$c:ident, $lua:ident $(, $arg:ident : $ty:ty)* $(,)?| $body:expr) => {{
            let char = char.clone();
            let f = lua.create_function(move |$lua, ($($arg,)*): ($($ty,)*)| {
    ...
    host_fn!("gather", |c, _lua| done(c.gather()));
    host_fn!("move", |c, _lua, x: i32, y: i32| done(c.move_to(x, y)));
    host_fn!("fight", |c, _lua| {
    ...
    host_fn!("rest", |c, _lua| done(c.rest()));
    host_fn!("deposit_item", |c, _lua, code: String, qty: u32| done(
        c.deposit_item(code, qty)
    ));
    host_fn!("deposit_all", |c, _lua| {
...
Add `WithdrawItem` to the row list:

```rust
    pub enum Intent { Move, Gather, Fight, Rest, DepositItem, WithdrawItem }
```
...
One row, after `deposit_item`:

```rust
    host_fn!("withdraw_item", |c, _lua, code: String, qty: u32| done(
        c.withdraw_item(code, qty)
    ));
```
...
- **Phase 1**, `docs/ARCHITECTURE.md`:
  - In the `core/` module table, change `step.rs`'s row to "The vocabulary:
    ...
    request format + outcome parsing on one struct; `define_intents!`
    generates the `Intent` enum and dispatch."
...
- **Phase 3**, `docs/ARCHITECTURE.md`: rewrite the "Adding things → A new
  action" bullet to the new checklist: wire struct + row (`core/src/wire.rs`),
  `OutcomeKind` variant if new shape (`core/src/step.rs`), `Character`
  wrapper, one `host_fn!` row (`src/lua.rs`), `def-action`
...
| 2 | Hermetic suite green with zero edits — `farm_copper::test_run_pass` exercises 5 of the 7 rows live; `farm_copper::test_plan_pass` exercises the stub registration path (any drift = nil-call = loud failure). |
...
- **Craft / equip / use / recycle**: each needs live verification (response
  ...
  smoke test like §6.7. Do not add speculative rows — the culling policy
  (`define_intents!`'s doc comment) stands.
...
- **No proc-macro, no trait objects, no change to `Step`/`Progress`, no
  cross-crate unification** of the `core` and `lua.rs` definitions — the
  sans-I/O boundary (`docs/ARCHITECTURE.md`) is load-bearing and preserved.
