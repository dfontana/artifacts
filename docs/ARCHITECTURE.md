...
flowchart TD
    subgraph authoring["AUTHORING — Fennel (where game logic is written)"]
        ...
        acq["<b>fennel/lib/acquire.fnl</b><br/>the acquisition generator: goal (item, qty) → plan-validated AST"]
    end
...
A workflow file evaluates to a **workflow module** — a table `{:doc <string?> :params <schema?> :build (fn [params ctx] ast-or-nil)}` — not a bare AST. `build` is a pure constructor of the workflow AST from its declared `:params` (coerced CLI/TUI inputs) and `ctx` (a read-only seed-state snapshot, including `ctx.bank` — a `{code → qty}` table of the account's bank holdings, empty when no bank data was supplied, layered on by `workflow::attach_bank`); it runs at load time (plan or run) and returns the tree built from the AST constructors (`seq`, `action`, `repeat_until`, `repeat_n`, `when_pred`). Example (`farm.fnl`): given `target=copper_rocks`, travel to that resource's tile, gather until inventory is full, travel to the bank, deposit everything. `build` returning **nil** signals "nothing to do" (the done-sentinel M7's `--until-done` loop stops on); a single-shot `plan`/`run` treats a nil build as a loud mistake. A bare-AST file (the old format) is rejected with a prescriptive error telling the author to wrap it in `{:build (fn [_ _] <ast>)}`.
...
### Generators — workflows whose `build` computes

`fennel/lib/acquire.fnl` is the generator layer (`plans/DYNAMIC_WORKFLOWS.md` §4): `(acquire.item code qty ctx opts)` works *backwards* from a goal ("hold ≥ qty of code") to a complete AST — withdraw what the bank already holds, then per remaining need pick the first eligible source in a declared **policy** order (default `[:craft :gather :fight :buy]`): recursively craft (inputs acquired leaves-first, each wrapped in a labelled `:group`), gather (a `has_item`-exit loop, split into bank trips when the target would overflow capacity), hunt (the `hunt.fnl` rest-gated fight loop), or buy from a gold-priced NPC within `opts.gold_budget`. No eligible source is a **loud build-time error** naming every rejected source with its reason ("gather: iron_rocks needs mining 10 (have 4); buy: 480 gold needed exceeds budget 100") — the CLI/TUI surface it verbatim.

Two properties make this safe. **Purity**: `acquire.item` reads only its args, `ctx`, and pure reference-data host fns (`item_sources`, `recipe`, `find_tile`, `monster_stats`, `simulate_fight`) — never a live host fn — so generation is identical in plan and run invocations and testable against fixtures (`tests/acquire.rs`). **The plan pass is the validator**: the generator tracks projected inventory/bank/gold/position as it emits (anchoring every `find_tile` on the itinerary, not the start), but its projections don't have to be perfect — a generated AST is planned exactly like a hand-written one, so a wrong generation surfaces as a blocker before anything runs, never as a silent lie.

`fennel/workflows/craft.fnl` is the flagship user of the library and the proof that a generator is *just a workflow module*: one required `:item` param, a defaulted `:qty`, and a one-line `build` that calls `acquire.item`. `artifacts plan fennel/workflows/craft.fnl <char> item=copper_dagger` prints the full predicted itinerary; `run` executes it.

## How Fennel maps back to Rust

### The host bridge — `src/lua.rs`

`setup_lua` builds an mlua state: it loads the vendored Fennel compiler, evaluates the five lib files, and installs a `host` table of Rust functions the Fennel layer calls. Two distinct sets of host functions:

- **Always present (pure computation):** `cooldown_cost`, `path_hops`, `gather_yield`, `resource_level`, plus the combat trio `monster_stats`, `simulate_fight`, `find_tile`, and `recipe` (the `craft` :sim's input-consume + output-add). These back the `:cost`/`:sim` facets, so the plan pass needs no _character_. The computations are pure; the reference data some of them read (monster stats, map content incl. per-resource skill+drops, craft recipes) is fetched and cached up front by the CLI bootstrap (both the no-arg and character `plan` invocations fetch the overworld map + monster data + `/items` recipes), but the simulation itself does no I/O. The NPC merchant catalog (`/npcs/items` → `data::NpcItemData`, keyed by item code) is cached the same way — its prices are **static**, changing only on a game patch, which is exactly why it's cacheable where the Grand Exchange isn't. The four reference datasets (resources, monsters, recipes, npc items) are also inverted **once at registration time** into `data::SourceIndex` — item code → every source for it — behind **`host.item_sources(code)`** (`{:craftable :resources :monsters :npcs}`, `DYNAMIC_WORKFLOWS` §5.5), the acquire generator's lookup. The source lists are sorted on the Rust side (resources by rate/level/code, monsters by rate/code, npcs by price/npc) so generated workflows are deterministic; the call errors loudly only when *no* dataset was loaded, while an unknown item returns empty lists — "unsourceable" is the generator's error to report, with the context (policy, budget, skills) only it has. `find_tile` takes an optional explicit anchor (`host.find_tile(kind, code, {:x :y}?)`) that overrides both the live-position (run) and seed-origin (plan) defaults — a generator threading an itinerary passes the previous destination so "nearest bank" resolves from where the character *will* be. Deliberately **not** cached: Grand Exchange order books (a live player marketplace — a TTL cache would be silently wrong; GE :sims take an author-supplied price hint instead), task definitions (their reward/assignment nondeterminism isn't fixable with reference data — a display concern for TUI v2), and the account's **bank holdings** (`/my/bank/items` → `data::BankData`) — live account state that any of the account's characters can change between (or during) invocations, the same reasoning class as the GE order book, so it is always fetched fresh, never TTL-cached (M4, `plans/DYNAMIC_WORKFLOWS.md` §5.6). `load_live_context` fetches it once per CLI/TUI invocation and it is surfaced two ways: `ctx.bank` (quietly empty when absent — a generator-facing convenience) and `host.bank()` (an always-registered, loud-when-unloaded host fn returning a fresh `{code → qty}` snapshot, for logic that wants a start-of-run read).
...
- **A new generator** (a workflow whose `build` computes its AST from a goal) → a Fennel library beside `fennel/lib/acquire.fnl`, plus a thin workflow module like `craft.fnl` that fronts it. Decompose it the way acquire does (`plans/DYNAMIC_WORKFLOWS.md` §4.3): **emitters** — reuse acquire's per-source patterns (bank-withdraw, gather/hunt loops, npc-buy) or `acquire.item` itself for any "obtain X" sub-goal; **oracles** — pure host lookups (`host.item_sources`, `host.recipe`, `host.simulate_fight`) plus `ctx`, never live host fns; **policies** — an explicit, overridable ranking (an `opts` table), not hidden heuristics. Keep `build` pure and let the plan pass validate the output — a generator never needs to be perfect, only honest enough that a wrong emission plans as a blocker. No Rust changes unless it needs new reference data.
