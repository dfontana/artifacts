...
mod common;
use common::{char_json, fight_win, make_map, response, spawn_mock, INV_MAX};
...
fn run_and_capture(
    ...
    let (character, _view, handle) = spawn_mock(driver, initial);
    let lua = setup_lua(LuaSetupOptions {
        character: Some(character),
        map: Some(test_map()),
        monsters,
        progress: Some(log.clone()),
        ..Default::default()
    })
    .expect("setup_lua");
    ...
    let run_fn: LuaFunction = interp.get("run").unwrap();
